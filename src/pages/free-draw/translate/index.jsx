...
const Canvas = memo(() => {
  ...
  const handleCanvasWheel = withBatchedUpdatesThrottled((event) => {
    const { deltaX, deltaY } = event;
    appState.scrollX = appState.scrollX - deltaX;
    appState.scrollY = appState.scrollY - deltaY;
    staticCanvasRef.current.style.transform = `translate(${appState.scrollX}px, ${appState.scrollY}px)`;
    if (timerId.current) {
    ...
    timerId.current = setTimeout(() => {
      ...
      context.clearRect(0, 0, canvas.width, canvas.height);
      renderScene(staticCanvasRef.current, appState);
      staticCanvasRef.current.style.transform = `translate(0px, 0px)`;
    }, 300);
  });
  return (
